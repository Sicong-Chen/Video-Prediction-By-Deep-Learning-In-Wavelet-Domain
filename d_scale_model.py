    ...
    def define_graph(self):
        ...
        with tf.name_scope('setup'):
            ...
            with tf.name_scope('full-connected'):
                ...
                # previous code: self.fc_layer_sizes: [512, 256, 1]
                self.fc_layer_sizes.insert(
                    0, (last_out_height / 2) * (last_out_width / 2) * self.conv_layer_fms[-1])
                # (last_out_height / 2) * (last_out_width / 2) * self.conv_layer_fms[-1] = 64
                # previous code: self.fc_layer_sizes: [64, 512, 256, 1]
                ...
                fc_pop_means =[]
                fc_pop_vars=[]
                for i in xrange(len(self.fc_layer_sizes) - 1):  #len(self.fc_layer_sizes) = 4, self.fc_layer_sizes = [25600, 1024, 512, 1]
        ...
        def generate_predictions():
            ...
            with tf.name_scope('calculation'):
                ...
                input = self.input_frames
                # if c.DOLAPLACIAN==1 and self.scale_index > 0:  '''Wavelet,c.DOLAPLACIAN == 0 already'''
                ...
                input = [inputs[:,0:Height/2,0:Length/2,0:Width/2],
                ...
                with tf.name_scope('convolutions'):
                    for i in xrange(len(conv_ws)):
                        ...
                        preds = tf.nn.conv2d(
                            #input[cube], conv_ws[i], [1, 1, 1, 1], padding=c.PADDING_D)
                            input, conv_ws[i], [1, 1, 1, 1], padding=c.PADDING_D)
                        if c.BATCH_NORM:
                ...
                with tf.name_scope('fully-connected'):
                    for i in xrange(len(fc_ws)):
                        #import pdb; pdb.set_trace()
                        # previous code: preds: TensorShape([Dimension(None), Dimension(64)])
                ...
                #preds_cubes.append(preds)

            #return preds_cubes  #get preds of each cube,return list[[1],[2],[3],[4],[5],[6],[7],[8]]
            return preds
