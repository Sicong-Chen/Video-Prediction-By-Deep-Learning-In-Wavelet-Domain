...
class AVGRunner:
    ...
    def train(self):
        ...
        while True:
            Teach_Prob = np.maximum(0,1-float(self.global_step)/20000)
    ...
    def test(self):
        ...
        psnr_all = np.array([])
        psnr_all_frame = np.zeros([1,self.num_test_rec])
        for i in xrange(0,c.TEST_INDEX.shape[0]-c.TEST_ALL*c.BATCH_SIZE,c.BATCH_SIZE):

            batch = get_test_batch(c.BATCH_SIZE, num_rec_out=self.num_test_rec,Indexs=c.TEST_INDEX[i:i+c.BATCH_SIZE])
            psnr,psnr_frame = self.g_model.test_batch(
                    batch, self.global_step, num_rec_out=self.num_test_rec, IndOffset = i, discriminator=(self.d_model if c.ADVERSARIAL else None),save_imgs=c.SAVE_IMG)
            psnr_all = np.append(psnr_all,psnr)
            psnr_all_frame = psnr_all_frame+psnr_frame


        psnr_all_avg = psnr_all.mean()
        psnr_all_frame = psnr_all_frame/(i+1)*c.BATCH_SIZE
        print 'avg_psnr    : ', psnr_all_avg
        print 'avg_psnr_frame    : ', psnr_all_frame
...
def main():
    ...
    try:
        opts, _ = getopt.getopt(sys.argv[1:], 'l:t:r:a:n:OTHA',
                                ['load_path=', 'test_dir=', 'recursions=', 'adversarial=', 'name=',
                                 'overwrite', 'test_only', 'help', 'test_all' 'stats_freq=', 'summary_freq=',
